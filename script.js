/* =========================================================
   ...
   ✓ Nodes / antinodes
   ✓ Play / pause / reset
   ✓ Runs indefinitely until paused or reset
...
function chooseStandingHighlights() {
    ...
    const particles = standingState.particles;
    ...
    const separation = 3;
    const centre = standingState.width / 2;
    let best = null;

    for (let i = 0; i < particles.length - separation; i++) {
        const p1 = particles[i];
        const p2 = particles[i + separation];

        const x1 = p1.canvasX - standingState.leftMargin;
        const x2 = p2.canvasX - standingState.leftMargin;

        if (getStandingLoopIndex(x1) !== getStandingLoopIndex(x2)) continue;

        const a1 = getStandingLocalAmplitude(x1);
        const a2 = getStandingLocalAmplitude(x2);
        if (a1 < standingState.amplitude * 0.15 || a2 < standingState.amplitude * 0.15) continue;

        const pairCentre = (p1.canvasX + p2.canvasX) / 2;
        const distance = Math.abs(pairCentre - centre);

        if (!best || distance < best.distance) best = { p1, p2, distance };
    }

    if (!best) {
        const i = Math.min(Math.floor(particles.length * 0.45), particles.length - separation - 1);
        best = { p1: particles[i], p2: particles[i + separation] };
    }

    if (best.p1) {
        best.p1.highlighted = true;
        best.p1.color = 'green';
        standingState.greenIndex = best.p1.index;
    }

    if (best.p2) {
        best.p2.highlighted = true;
        best.p2.color = 'red';
        standingState.redIndex = best.p2.index;
    }
}
...
function updateProgressiveSliderDisplays() {
    const values = {
        progressiveAmplitudeValue: progressiveState.amplitude.toFixed(0),
        progressiveFrequencyValue: `${progressiveState.frequency.toFixed(1)} Hz`,
        progressiveWavelengthValue: progressiveState.wavelength.toFixed(0)
    };
...
function updateStandingSliderDisplays() {
    const values = {
        standingAmplitudeValue: standingState.amplitude.toFixed(0),
        standingFrequencyValue: `${standingState.frequency.toFixed(1)} Hz`,
        standingWavelengthValue: standingState.wavelength.toFixed(0)
    };
    Object.entries(values).forEach(([id, value]) => {
        const element = getElement(id);
        if (element) element.textContent = value;
...
function playProgressiveWave() {
    if (progressiveState.running) return;
    progressiveState.running = true;
    progressiveState.lastTimestamp = null;
    progressiveState.animationFrame = requestAnimationFrame(progressiveAnimationLoop);
...
function playStandingWave() {
    ...
    standingState.lastTimestamp = null;
    standingState.animationFrame = requestAnimationFrame(standingAnimationLoop);
    updatePlayPauseButtons();
}
...
function resetProgressiveWave() {
    ...
    progressiveState.time = 0;
    progressiveState.amplitude = PHYSICS.DEFAULT_AMPLITUDE;
    progressiveState.frequency = PHYSICS.DEFAULT_FREQUENCY;
    progressiveState.wavelength = PHYSICS.DEFAULT_WAVELENGTH;
    setSliderValue("progressiveAmplitude", progressiveState.amplitude);
    setSliderValue("progressiveFrequency", progressiveState.frequency);
    setSliderValue("progressiveWavelength", progressiveState.wavelength);
...
function resetStandingWave() {
    ...
    standingState.amplitude = PHYSICS.DEFAULT_AMPLITUDE;
    standingState.frequency = PHYSICS.DEFAULT_FREQUENCY;
    standingState.wavelength = PHYSICS.DEFAULT_WAVELENGTH;
    setSliderValue("standingAmplitude", standingState.amplitude);
    setSliderValue("standingFrequency", standingState.frequency);
    setSliderValue("standingWavelength", standingState.wavelength);
...
function removeSpeedControls() {
    ["progressiveSpeed", "standingSpeed"].forEach(id => {
        ...
        const wrapper = slider.closest(".slider-group, .control-group, .control-row, .parameter-control");
        if (wrapper) wrapper.style.display = "none";
    });
}


function syncInitialSliderValues() {
    const sliders = [
        ["progressiveAmplitude", progressiveState.amplitude],
        ["progressiveFrequency", progressiveState.frequency],
        ["progressiveWavelength", progressiveState.wavelength],
        ["standingAmplitude", standingState.amplitude],
        ["standingFrequency", standingState.frequency],
        ["standingWavelength", standingState.wavelength]
    ];
...
function setupProgressiveControls() {
    const play = getElement("progressivePlay");
    const pause = getElement("progressivePause");
    const reset = getElement("progressiveReset");

    if (play) play.addEventListener("click", playProgressiveWave);
    if (pause) pause.addEventListener("click", pauseProgressiveWave);
    if (reset) reset.addEventListener("click", resetProgressiveWave);

    bindRange("progressiveAmplitude", value => {
        progressiveState.amplitude = value;
        updateProgressiveSliderDisplays();
        drawProgressiveWave();
    });
    bindRange("progressiveFrequency", value => {
        progressiveState.frequency = value;
        updateProgressiveSliderDisplays();
        drawProgressiveWave();
    });
    bindRange("progressiveWavelength", value => {
        progressiveState.wavelength = value;
        updateProgressiveSliderDisplays();
        drawProgressiveWave();
    });
    bindToggle("progressivePhaseToggle", value => {
        progressiveState.showPhase = value;
        drawProgressiveWave();
    });
    bindToggle("progressiveEnergyToggle", value => {
        progressiveState.showEnergy = value;
        drawProgressiveWave();
    });
    bindToggle("progressiveDirectionToggle", value => {
        progressiveState.showDirection = value;
        drawProgressiveWave();
    });
}
...
function setupStandingControls() {
    const play = getElement("standingPlay");
    const pause = getElement("standingPause");
    const reset = getElement("standingReset");

    if (play) play.addEventListener("click", playStandingWave);
    if (pause) pause.addEventListener("click", pauseStandingWave);
    if (reset) reset.addEventListener("click", resetStandingWave);

    bindRange("standingAmplitude", value => {
        standingState.amplitude = value;
        updateStandingSliderDisplays();
        chooseStandingHighlights();
        drawStandingWave();
    });
    bindRange("standingFrequency", value => {
        standingState.frequency = value;
        updateStandingSliderDisplays();
        drawStandingWave();
    });
    bindRange("standingWavelength", value => {
        standingState.wavelength = value;
        updateStandingSliderDisplays();
        chooseStandingHighlights();
        drawStandingWave();
    });
    bindToggle("standingNodesToggle", value => {
        standingState.showNodes = value;
        drawStandingWave();
    });
    bindToggle("standingPhaseToggle", value => {
        standingState.showPhase = value;
        drawStandingWave();
    });
    bindToggle("standingEnergyToggle", value => {
        standingState.showEnergy = value;
        drawStandingWave();
    });
}
...
/* =========================================================
   56. PLAY / PAUSE BUTTON STATE
   ========================================================= */

function updatePlayPauseButtons() {
    const buttons = [
        ["progressivePlay", !progressiveState.running],
        ["progressivePause", progressiveState.running],
        ["standingPlay", !standingState.running],
        ["standingPause", standingState.running]
    ];
    buttons.forEach(([id, enabled]) => {
...
function updateAccessibilityStatus() {
    ...
    const progressiveText = progressiveState.running
        ? "Progressive wave running"
        : "Progressive wave paused";

    const standingText = standingState.running
